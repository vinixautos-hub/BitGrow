      ...
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        {!session ? <>
          ...
          <div ref={dropRef} style={{position:"relative"}}>
            ...
            {drop&&(
              <div style={{position:"fixed",top:68,right:16,width:270,background:"#0f1117",border:"1px solid #1e2030",borderRadius:16,boxShadow:"0 24px 70px rgba(0,0,0,.8)",zIndex:400}}>
                <div style={{padding:"16px 18px 14px",background:"#080910",borderBottom:"1px solid #1e2030",borderRadius:"16px 16px 0 0"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:40,height:40,borderRadius:"50%",background:"linear-gradient(135deg,#fbbf24,#f97316)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:18,color:"#07080f"}}>{(p?.first_name||p?.name||"U")[0].toUpperCase()}</div>
                    <div><div style={{fontWeight:700,fontSize:14,color:"#e8eaf0"}}>{p?.name||"User"}</div><div style={{fontSize:11,color:"#4b5563"}}>{session?.user?.email}</div></div>
                  </div>
                </div>
                {[["📊","Overview",()=>goTab("overview")],["💰","Invest",()=>goTab("invest")],["💸","Withdraw",()=>goTab("withdraw")],["📋","History",()=>goTab("history")],["👥","Referrals",()=>goTab("referrals")]].map(([icon,label,fn])=>(
                  <div key={label} onClick={fn} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 18px",cursor:"pointer",transition:"background .12s"}} onMouseEnter={e=>e.currentTarget.style.background="#111218"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <span>{icon}</span><span style={{fontSize:13,fontWeight:600,color:"#e8eaf0"}}>{label}</span>
                  </div>
                ))}
                {isAdmin&&<div onClick={()=>{navigate("admin");setDrop(false);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 18px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#111218"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><span>⚙️</span><span style={{fontSize:13,fontWeight:600,color:"#fbbf24"}}>Admin Panel</span></div>}
                <div style={{borderTop:"1px solid #1e2030",margin:"4px 0 0"}}>
                  <div onClick={()=>{logout();setDrop(false);}} style={{display:"flex",alignItems:"center",gap:10,padding:"13px 18px",cursor:"pointer",borderRadius:"0 0 16px 16px"}} onMouseEnter={e=>e.currentTarget.style.background="#1a0808"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                    <span>🚪</span><span style={{fontWeight:700,fontSize:13,color:"#ef4444"}}>Sign Out</span>
                  </div>
                </div>
              </div>
            )}
...
export default function App() {
  ...
  const navigate=p=>{setPageHistory(h=>[...h,page]);setPage(p);window.scrollTo(0,0);};
  const goBack=()=>setPageHistory(h=>{const prev=h[h.length-1]||"home";setPage(prev);return h.slice(0,-1);});
  const logout=()=>{_session=null;setSession(null);setProfile(null);setPage("home");setPageHistory([]);localStorage.removeItem("bg_session");};
  ...
  return (
    <div style={{fontFamily:"system-ui,-apple-system,sans-serif",minHeight:"100vh",background:"#07080f",color:"#e8eaf0"}}>
      ...
      `}</style>
      <Nav page={page} navigate={navigate} goBack={goBack} pageHistory={pageHistory} session={session} profile={profile} logout={logout} isAdmin={isAdmin} setDashTab={setDashTab} notifCount={notifCount} setNotifCount={setNotifCount}/>
      ...
      {page==="dashboard" && session       && <Dashboard session={session} profile={profile} dashTab={dashTab} setDashTab={setDashTab}/>}
